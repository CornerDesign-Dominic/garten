...
export function Header() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-emerald-900/10 bg-[#f6fbf3]/95 backdrop-blur-sm">
        ...
        <nav aria-label="Hauptnavigation" className="flex items-center gap-6 sm:gap-8">
          {navigationItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="text-sm font-medium text-zinc-700 transition-colors hover:text-emerald-800"
            >
              {item.label}
            </Link>
          ))}
